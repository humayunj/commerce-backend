...
export default class ProductsDAO {
    ...
    static Product =  MongooseService.getInstance().getMongoose().model<IProduct>("Product",ProductsSchema)
    ...
    async find(filter:mongoose.FilterQuery<IProduct>,limit:number,skip:number) : Promise<IProduct[]> {
        const p = await ProductsDAO.Product.find(filter).limit(limit).skip(skip).exec();
        return p;
    }
    async findOne(filter:mongoose.FilterQuery<IProduct>) : Promise<IProduct | null> {
